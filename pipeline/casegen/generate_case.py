...
def generate_case(params_path, output_dir):
    p = load_params(params_path)
    os.makedirs(output_dir, exist_ok=True)

    print("Building bottom case...")
    case = make_bottom_case(p)
    export_model(case, "bottom_case", output_dir)
    export_svg_preview(case, "bottom_case", output_dir)

    print("Building positioning plate...")
    plate = make_plate(p)
    export_model(plate, "plate", output_dir)
    export_svg_preview(plate, "plate", output_dir)
...
def main():
    ...
    generate_case(json_path, os.path.dirname(json_path) or '.')
